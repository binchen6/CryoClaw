...
- 本地跑过 `npm test` 全绿（基线 439 pass / 0 fail / 4 skipped）；新增/改动 chat-ui 控制器或纯函数时请在 `chat-ui/ui/src/**/*.test.ts` 同步补测试
- 一个 PR 只做一件事
...
- `npm test` is fully green (baseline 439 pass / 0 fail / 4 skipped); add tests under `chat-ui/ui/src/**/*.test.ts` for new/changed chat-ui controllers or pure functions
- One PR, one thing
