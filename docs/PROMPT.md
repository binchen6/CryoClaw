# CryoClaw UI React 重写任务

当前项目是 CryoClaw——一个基于 Electron 的桌面应用，它将 openclaw gateway 封装为独立的可安装程序。参考 CLAUDE.md 和 docs/architecture.md 了解项目基本信息。使用 `tokei -o json chat-ui/ setup/ settings/` 命令可以看到当前 UI 层的代码行数分布。
...
4. **IPC 契约不变**：`window.cryoclaw.*` 的所有方法签名和调用约定不变。这是与 Main Process 的唯一接口
