...
- 测试基线 **434 pass / 0 fail / 4 skipped**（vitest 94 + node 64/68 + chat-ui 228 + scripts 48 + tsc typecheck；
  chat-ui 228 含 markdown 渲染引擎增强 3 用例（GFM 任务列表/原始 HTML 转义/超长文本退化）；0 fail 为硬指标）。
- 历史优化阶段 1–22 全部完成并逐版发版至 v2026.811.0（见历史档案）。
- 已开源发布至 GitHub（binchen6/CryoClaw，AGPL-3.0-only）；发布时以全新干净历史快照推送，旧本地历史（含已作废的 kimi-claw REFRESH 凭证）不出仓；`.env.build` 已转 gitignored，模板见 `.env.build.example`。
