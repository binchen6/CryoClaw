...
**当前状态**：
- 重设计工程 **R1–R92 完成**（R92：任务模块统计/搜索/筛选/自动刷新重构；R91：扩展页市场与更新体系（插件检查更新/详情/市场发现 + 统一推荐算法）+ 三轮审查 + 性能批次；R86：更新链路三修 + 安装器可见进度换装 + 记忆联想自定义服务商/API Key + 模型下拉 + WebBridge 误报根治；R85：QA 实拍三连修；R84：.btn 收敛 + 流式列宽；R83：统一工具卡；R82：记忆页内核全景；R74：安全纵深（OAuth 授权链接域校验 + ClawHub registry scheme 守卫）+ --shadow-dialog 令牌化 + 死代码清理 + 文档同步归档；R73：两轮全面 debug 审查四路 36 项；R72：内核 2026.9.3 适配（Node runtime 22→24）+ 词条覆盖/长文案溢出审查 + 性能与内存批次 + 死代码清理；R71：故障路径审查——网关崩溃自动恢复；R70：交互级页面审查（新增发版固定步骤）+ 确认弹窗 Escape 修复；R69：无障碍批次——模态键盘关闭/焦点 + 可交互行键盘可达；R68：WebBridge 修复永久化——可自动更新的远端钉定清单；R67：第二轮功能/页面审查（P0 安装向导模型控件 + 9 项）；R66：WebBridge 钉定表随上游轮换 + 功能/页面审查 17 项修复；R65：WebBridge 供应链钉定；R64：三路全库审查 + UI 截图 QA；R63：MCP 页重叠修复 + 真静默更新换装；R62：回底按钮 + 消息对齐及时性；R61：内核问答卡片；R60：设置页 MCP 与 Hooks + 四路全库审查；R59–R58：在途输出恢复/对齐用量，详见工程记录），最新发版 **v2026.916.0**。
...
- 测试基线 **1205 pass / 0 fail / 4 skipped**（vitest 181 + node 225 + chat-ui 718 + scripts 81；2026-09-16 实测，0 fail 为硬指标）。
...
### R86 · 更新链路三修 + 记忆联想自定义服务商/API Key + 模型下拉 + WebBridge 误报根治

- **审查方法**：先取证后动手——读生产机 `~/.openclaw/logs/app.log` 还原 2026.912.5→2026.913.0 真实更新时间线（差分两次失败：ERR_CONNECTION_RESET + sha512 校验不匹配，各付几十 MB 才回退全量；下载启动 100ms 后一次渲染层 check 把进度打回 available；spawn 安装器 150ms 后老应用正常退出，但静默换装跑足 10.5 分钟零 UI），再拆 electron-updater 6.8.9 源码（差分进度转换器复制阶段不发事件）与 app-builder-lib 26.7.0 NSIS 模板（--updated 跳页序列）逐环验证。
- **更新器修复（三处）**：① `disableDifferentialDownload = true`——GitHub CDN 分段 Range 请求弱网高失败率，失败即双份流量，全量单连接可断点续传且进度平滑；② 状态机新增 `download-start` 事件（downloadAppUpdate 先发 0% 占位进度，UI 即刻进下载态，消除首个 progress 事件前数秒静默），dialog 进度行 total=0 时只显示「下载中」；③ `checkAppUpdate()` 在 downloading/downloaded 态拒绝并发检查（顺带修掉 downloaded 态误 check 清空「重启安装」按钮的旧疾）。
- **安装器换装改可见进度**：spawn 参数 `[--updated --force-run]` 去掉 `/S`——跳页宏齐备后非静默只剩单页 instfiles 进度窗（Welcome/安装模式/目录/Finish 全 Abort，onFinishPagePre 自动拉起新版）；R63「无 /S 弹向导」结论过期勿引用。守护测试同步改写（app-update-notify.test.ts）。教训沉淀 gotchas #106/#107。
- **记忆联想自定义服务商 + API Key**：服务商下拉新增「已配置的服务商」optgroup（models.providers 中 api=openai-completions/openai-responses 或带 baseUrl 的 key，内核 isOpenAICompatibleMemoryProvider 原生支持，baseUrl 自动回落 provider 配置）；新增 API Key password 字段，三态保存语义（null=透传脱敏哨兵 / ""=清除 / 非空=写入，内核 config.patch 写侧自动还原哨兵，已核实 sensitive-paths 覆盖 memory.search.remote.apiKey）+ 清除/撤销按钮；normalizeProvider 放行任意非空字符串（此前会把自定义 key 静默归一成 openai）。
- **记忆模型字段下拉化**：session/dreaming/active-memory 三个 model 字段从自由文本改为下拉（renderModelOptionsGrouped 复用，选项=已配置模型组合键，「跟随默认模型」空值，存量裸 id 按后缀匹配显示、匹配不到追加额外选项防丢失；内核 parseModelRef 双格式兼容已核实）。
- **WebBridge 误报根治（每次启动弹「连接你的常用浏览器」+ 偶发连不上）**：① 扩展启用检测从仅查 Default profile 升级为全 profile 扫描（listProfileSubdirs：Local State info_cache 权威 → 目录名正则 → Default 兜底；isExtensionPresentInAnyProfile），needs-repair 与 pill-repair ALREADY_OK 双口径统一；② daemon /status 的 extension_version 非空即判「已连接」短路隐藏 pill（Secure Preferences 是懒刷新磁盘快照，按它判定必误报；异常路径同享短路）；③ 新增 ensureWebbridgeDaemonRunning 启动保活（webbridge 模式 + binary 存在 + /status 不通才 detached 拉起，gateway-ready 触发，成功后延迟广播 state-changed）；④ pill 可见期间渲染层 30s 自愈复判（隐藏态零开销）。
- **代码审查（三轮）**：① 独立盲审 1 个 P1/P2 级为零，3 个 P3（apiKey 输入未取消 Kimi 一键挂起、optgroup 未过滤内核不支持的 provider、pill-repair ALREADY_OK 仍 Default-only）全修；② 终审轮交叉一致性（securePreferencesPath 签名、MemorySearchView.provider 放宽、require() 懒加载 node --test 兼容、i18n AST 级键集比对 1234/1234 零孤儿、Lit select .value+?selected 首渲染语义）全绿；③ 残留 UX 观察一条（清除挂起时输入新 key）补一行撤销逻辑。
- **验证**：全量 1119 pass / 0 fail / 4 skipped（vitest 161 + node 207 + chat-ui 670 + scripts 81；新增 browser 多 profile 2 用例、memory lib apiKey 三态 3 用例、download-start 2 用例）；双 tsc 零错误；tab-memory.lib/i18n/app-update-notify 断言同步更新。

### R91/R92 · 扩展页市场与更新体系 + 任务模块重构 + 三轮审查 + 性能批次

- **扩展页（R91，本 session 主题）**：① 插件更新——`plugin-store:check-updates`（内核 `plugins update --dry-run --all`，主进程剥离 ANSI + 解析结果行）/ `plugin-store:update`（单项/全部，完成后重启网关入口）；② 插件/技能详情页——`plugins inspect <id> --json` 的能力/渠道/Provider/工具/钩子/安装记录全展示，技能详情拉 ClawHub `/api/v1/skills/<slug>` 含净化 Markdown readme；③ 插件市场浏览——`plugin-store:market-browse` 按 7 分类×2 关键词×2 family 聚合 ClawHub 包搜索，CLI 回退；④ 推荐与发现算法（`chat-ui/ui/src/ui/market/market-recommend.ts` 纯函数库）：trust 0.40（官方 1 / verified 0.7 / known 0.4 / 其他 0.15）+ popularity 0.35（log10 压缩、5 万封顶）+ freshness 0.25（30 天半衰），个性化 hints（已装插件/技能 token，词边界匹配 ×1.35），蛇形类目多样化（无 3 连同类），技能侧经 skillToMarketItem 复用同一引擎；⑤ 布局——`--ext-column` min(92%,1200px)→min(94%,2400px)、`--page-column` 1080→1600（宽屏占窗 ≥85%），技能/市场列表网格化（auto-fill minmax）。
- **任务模块（R92）**：统计条（4 chip 联动状态过滤）、防抖搜索（title/kind/runtime/agentId/sessionKey）、来源+Agent 筛选、30s 自动刷新（leave hook + 视图自查双保险，visibility 门控）、失败详情展开/收起、错误重试、最近完成显示全部、≤720px 响应式；过滤/统计全部抽成可测纯函数（deriveTaskStats/filterTasksByQuery/collectAgentIds/filterTasksByRuntime）。
- **三轮审查（并行子智能体）**：轮 1 主进程 8 项（P1：三处 CLI spawn 缺 OPENCLAW_STATE_DIR 的 HOME 歧义；P2：pairing 超时未杀进程树、before-quit 清理早于 gateway 停止、剪贴板多文件 NUL 未拆、kimi-code 验证劫持共享代理 token、ensureAuthProxy 无条件重建、save-advanced 无关改动触发重启、spawn 失败白等 5s）；轮 2 前端 4 项（webbridge 轮询句柄泄漏、幽灵 pending session label、memory 展开迟到覆写、末位裸字号）；轮 3 新代码 7 项（P1：更新/安装/卸载互斥缺失致内核 CLI 并发改写插件目录；P2：ticker 绕过 leave hook 泄漏、before-quit finally 截断、registry 读侧 scheme、死过滤、loading 残留、缺键）+ i18n 双侧对称校验——全部修复。
- **性能批次**：插件清单缓存 60s→10min、市场浏览 5min TTL（原每次 28 路 HTTP）、技能列表 5min 缓存、gateway 守护进程清理每会话一次（原每次 start 付 2 次 schtasks spawn）、健康轮询前 5s 150ms 快轮、extension-mirror 复制异步化（原同步复制卡主进程 IPC 1-10s）、工具流淘汰段 12k 字符截断、hljs/katex 拆出 vendor-misc（modulepreload 不再每次窗口创建强制加载 340KB；CSS 模块钉回 vendor-misc 防 facade import 反拖，gotcha #109）。
- **kimi-code 验证去劫持**：代理新增逐请求 `x-cryoclaw-verify-key` 头（转发前剥除），verifyKFC 与图像探测（extraHeaders 仅 127.0.0.1 前缀注入）全链路走该头，取代旧的全局 setProxyAccessToken 劫持-恢复模式（劫持窗口内生产流量 401 最长 ~15s）。
- **治理**：`.dev-app.log` 误提交出库 + gitignore；ipc-api.md 补 5 通道；测试基线 1205 pass / 0 fail / 4 skipped（vitest 181 + node 225 + chat-ui 718 + scripts 81）。

### R93 · 插件链路五连修（更新检查/市场浏览/manifest 告警/DEP0180/重复插件与误禁告警）+ 双轮审查

- **审查方法**：用户生产日志（clawhub.ai 443 连接超时 + market browse failed + manifest-metadata-scan 告警 + duplicate plugin id + tavily disabled-but-config）入手，逐条到 vendored 内核 asar 反查根因；修复后独立 review-agent 盲审 11 项发现（4×P1 + 7×P2）全部落地，其中最关键的是「Failed 行走 stderr 且内核 exit 1」——首版实现按 stdout 解析属死代码，真实内核探针证实后才改为 exec 拒绝路径附件解析。
- **更新检查（check-updates）**：内核 `plugins update --dry-run --all` 在 ClawHub 不可达时逐插件打 `Failed to check <id>: …`（stderr、exit 1）。execKernelCli 拒绝时附 stdout/stderr；handler 解析合并文本 → Electron 侧 HTTP 回退（marketApiBase detail 端点拿 latestVersion，comparePluginVersions 数字感知 + semver 预发布语义比对）→ 部分失败 success:true + `data.failed` 非阻塞提示（前端 checkNotice + i18n zh/en），全部失败给网络/Registry 配置指引。marketApiBase 默认链补构建期 CRYOCLAW_CLAWHUB_REGISTRY（此前 plugin-market 硬编码官方域名，构建期镜像不生效）。
- **市场浏览（market-browse）**：回退链补 CN 镜像末级试探（cn.clawhub-mirror.com 当前无 packages API、404 秒回，镜像补齐后自动恢复）；终错从裸英文标记串改为可行动中文指引。market-detail 同样加镜像回退 + 镜像 404 时透传「主源不可达」而非误导性 404。实测确认 OPENCLAW_CLAWHUB_URL env 对内核插件命令无效（调用方显式 baseUrl 优先），不透传（透传反而会把 skills-only 镜像路由给内核安装路径弄坏安装）。
- **manifest 告警**：上游 openclaw 2026.9.3 把共享运行时库 dist/extensions/image-generation-core 放进插件扫描目录但不带 openclaw.plugin.json，内核 manifest-metadata-scan 每轮启动告警。打包期合成最小清单 `{id, enabledByDefault:false}`（实验验证 plugins list 输出零变化：内核消费方只读 providers/modelCatalog 等字段），verifyAsarContents 钉住。
- **DEP0180**：Electron 43 已知 bug（electron#47390，asar stat 用已弃用 fs.Stats 构造器）——gateway/execKernelCli/execClawhub 三处 spawn 前插 `--no-deprecation`（gotcha #111）。
- **wecom 重复插件**：用户经市场安装的插件（npm/projects 受管根）与 extension-mirror 铺的镜像副本同 id 冲突。reconcile 时用户副本优先：移除/跳过镜像副本（用户卸载市场版后下轮启动自动铺回，自愈）。新增 plugin-install-roots.ts 统一扫描 npm/projects（只认项目 package.json dependencies 声明且带插件清单的包——basename 兜底会把 wecom 项目的 45 个传递依赖污染成假 id）。
- **tavily 误禁（config-migration）**：migrateUnavailablePluginEntries 的 resolvable 集合漏了 npm/projects 安装根，把市场安装的插件（tavily 等）误禁并触发内核常驻 disabled-but-config 告警。修复四件套：① resolvable 补 npm/projects；② 一次性修复历史误禁（mistakenPluginDisableRepairDone 门控；仅非 channel 插件——channel 机器人可能是用户显式禁用的业务决策）；③ 确认不可解析的条目改为 config 暂存（cryoclaw.config.json savedPluginConfigs）+ 删除条目（消告警），重装时 plugin-store 自动恢复（restoreStashedPluginConfig，一次恢复全部命中、不覆盖已有 config、尊重显式禁用）；④ 降级模式——安装根读取硬失败（EBUSY 等）/sidecar 损坏时退回旧禁用语义（读不到 ≠ 没安装，key 丢失不可容忍），暂存写失败回滚内存删除（绝不"两边都没有"）。
- **代码审查（两轮）**：自查（单元素 Set 冗余、i18n 契约测试常量更名同步）+ 独立盲审 11 项全修：P1 ×4（stderr/exit1 死代码、stash 写失败仍落盘删除、一次性修复误启用户显式禁用的 channel 插件、registry env 透传潜在破坏安装）；P2 ×7（回退全 up-to-date 误报解析失败、预发布版本反转、损坏 sidecar 被覆盖、依赖包名污染、恢复只修首个+陈旧暂存覆盖、瞬时读失败误删、__proto__ stash key）。教训沉淀 gotchas #110/#111/#112。
- **验证**：全量 1225 pass / 0 fail / 4 skipped（vitest 187 + node 235+4 + chat-ui 722 + scripts 81；新增 plugin-install-roots.test.ts 5 用例、migration 4 用例、plugin-store Failed 行/版本比较 3 用例）；双 tsc 零错误。
