...
function getDiagStream(): fs.WriteStream {
  if (!diagStream) {
    ...
    diagStream = fs.createWriteStream(LOG_PATH, { flags: "a" });
    diagStream.on("error", () => { diagStream = null; });
  }
  return diagStream;
}

// 进程退出时 flush：end 后等 close，保证缓冲日志落盘（1s 超时兜底，退出路径不阻塞）
export async function closeDiagLogStream(): Promise<void> {
...
function ensureClawhubWrapper(nodeBin: string): void {
  ...
  if (IS_WIN) {
    ...
    const wrapper = [
      "#!/usr/bin/env bash",
      "# CryoClaw clawhub CLI - auto-generated, do not edit",
      `APP_NODE="${safeNode}"`,
      `APP_ENTRY="${safeEntry}"`,
