...
function withFakeHome(fn: (home: string) => void | Promise<void>): () => Promise<void> {
  return async () => {
    ...
    const orig = { h: process.env.HOME, u: process.env.USERPROFILE, a: process.env.CRYOCLAW_BROWSER_APPS_DIRS };
    process.env.HOME = process.env.USERPROFILE = home;
    process.env.CRYOCLAW_BROWSER_APPS_DIRS = path.join(home, "Apps-fake");
    try { await fn(home); } finally {
      process.env.HOME = orig.h; process.env.USERPROFILE = orig.u;
      if (orig.a === undefined) delete process.env.CRYOCLAW_BROWSER_APPS_DIRS;
      else process.env.CRYOCLAW_BROWSER_APPS_DIRS = orig.a;
    }
  };
}
...
test("BROWSER_TARGETS 仅 chrome / edge + isBrowserInstalled 看 Local State", withFakeHome((home) => {
  ...
  // isBrowserInstalled 按当前平台取 userDataDir，Windows 下要用 Win 路径
  const rel = process.platform === "win32" ? chrome.userDataDirWin : chrome.userDataDirMac;
  const ud = path.join(home, rel);
...
test("[mac] cleanExtensionBlocklist 移除 ID 但保留其它字段", { skip: process.platform !== "darwin" }, withFakeHome(async (home) => {
...
test("[mac] isExtensionPresentInChrome: disable_reasons=[] → true，非空数组 → false", { skip: process.platform !== "darwin" }, withFakeHome(async (home) => {
  const pd = path.join(home, chrome.userDataDirMac, chrome.profileSubdir);
  fs.mkdirSync(pd, { recursive: true });
