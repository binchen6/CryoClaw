...
// 固定基址端口；被占用时递增重试（最多 GATEWAY_CONTROL_MAX_PORT_ATTEMPTS 个），
...
export function resolveResourcesPath(): string {
  ...
  const target = process.env.CRYOCLAW_TARGET ?? `${process.platform}-${process.arch}`;
 ...
 * （历史上如 kimi-claw 依赖的 fs-ext，已于 R7 移除），dev 下若改用 runtime/node（ABI 不同）会触发
...
export function resolveDevBranchTag(): string {
  if (!process.env.CRYOCLAW_MULTI_INSTANCE) return "";
  try {
...
/** 检查 Setup 是否已完成（优先读 cryoclaw.config.json，兼容旧版） */
export function isSetupComplete(): boolean {
  // 新逻辑：cryoclaw.config.json 的 setupCompletedAt
  const cryoclawConfig = readCryoclawConfig();
  if (cryoclawConfig?.setupCompletedAt) return true;
