...
// openclaw 的 config observer 在每次读 openclaw.json 时会做健康体检：
//   1) 2026.4.x：看 logs/config-health.json 里的 lastKnownGood，fallback 读 openclaw.json.bak；
//      ≥2026.6.x：baseline 迁移到 SQLite 表 config_health_entries（旧 JSON 仅一次性迁移），
//      .bak 由内核自行按 .bak/.bak.1-4 环维护。
//   2) 用 baseline 与当前 cfg 做 size-drop / hash-mismatch 等判定，若 suspicious
//      则落一份 .clobbered.* 快照，并尝试从 .bak copy 回来。
// 当 CryoClaw 主进程绕过 openclaw 直写 openclaw.json 时 baseline 会停留在旧字节数，
// 于是每次 gateway 读配置都可能判定 size-drop → dump clobbered → I/O 雪崩。
//
// 修复：CryoClaw 每次直写 openclaw.json 后，做两件事（顺序无关）：
//   1) 同步用同样字节覆盖 openclaw.json.bak —— 这样无论 openclaw 走哪条 baseline
//      fallback（或从 .bak 恢复），对比/恢复的字节都与当前配置一致；
//   2) 删除 config-health.json 里该路径的 entry，让 openclaw 下次 read 时重建
//      lastKnownGood。≥2026.6.x 起该文件已被 SQLite 取代，此步对不存在/legacy
//      文件静默跳过，仅作防御。真正的关键是第 1 步。
...
/**
 * CryoClaw 直写 openclaw.json 后统一调用。合并 .bak 同步 + health-state 清理两步。
 */
