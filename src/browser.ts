...
// 退而求其次：<userDataDir>/Local State 存在（Chromium 启动时创建，CryoClaw 不会写）。
// Windows：只用 <userDataDir>/Local State（Chromium 至少启动过一次）。
// 注意：不能用「user data dir 是否存在」判定——CryoClaw 写 External Extensions JSON 时
// 会自己创建 user data dir 子目录，造成"幽灵安装"假象。
//
// 测试钩子：env CRYOCLAW_BROWSER_APPS_DIRS=":分隔" 可覆盖 macOS app 搜索路径
// （绕开宿主机 /Applications 里真实装的浏览器对单元测试的污染）。
function macAppSearchDirs(): string[] {
  const override = process.env.CRYOCLAW_BROWSER_APPS_DIRS;
...
/**
 * CryoClaw 用 Chrome External Extensions 协议宣告本地 CRX 安装包：
...
// 用户配置不是 CryoClaw 私有数据，损坏代价大。
function atomicWriteFile(targetPath: string, data: string): void {
  const tmpPath = `${targetPath}.cryoclaw.tmp-${process.pid}`;
...
// 单一默认浏览器策略：CryoClaw 只在系统默认浏览器（Chrome/Edge）上装扩展。
...
// 后者只是 CryoClaw 写给 Chrome 的"建议"，前者反映 Chrome 是否真的把扩展加载进来了。
...
function applyWebbridgeMode(config: CryoclawConfigShape): any {
  return {
    ...config,
    plugins: {
      ...(config.plugins ?? {}),
...
export function detectBrowserMode(config: CryoclawConfigShape): BrowserMode {
