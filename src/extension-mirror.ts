 ...
 *
 * CryoClaw 不再使用 openclaw 的 `bundled-channel-entry` 契约（那条路径需要 138 行
...
// 曾经被 CryoClaw mirror，但已经迁移到 openclaw 内置 vendor / bundled 路径的
// plugin id。旧用户的 ~/.openclaw/extensions/<id>/ 仍残留旧版本，与 openclaw
// stock / bundled 同时存在会触发 duplicate plugin id 警告，启动时静默删除一次
// 即可。
//
//   - qqbot：2026.4.5 起由 openclaw 官方 vendor，CryoClaw 不再 ship
...
function reconcileOne(pluginId: string, mirrorDir: string, userDir: string): ReconcileOutcome {
  ...
  // 两个维度都相等才 skip：
  //   - 插件上游版本（pkg.version）
  //   - CryoClaw 构建期重写的入口（pkg.openclaw.extensions[0]）
  // 后者是为了让 CryoClaw 换 bundle 策略（比如把 `.ts` 入口替换成
  // `./dist/cryoclaw-bundle.mjs`）能触发 reconcile，即使 plugin 版本没变。
