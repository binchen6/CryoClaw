...
// CryoClaw 自有 CLI wrapper 的标记字符串，与 cli-integration.ts 保持一致。
// 旧版（OneClaw 时代）wrapper 的标记同样视为自有 wrapper，否则会把上一代
// 托管 wrapper 误判为外部全局安装。
const CLI_MARKERS = ["CryoClaw CLI", "OneClaw CLI"];
...
// 检测单个命令的全局安装路径，排除 CryoClaw 自有 CLI wrapper
async function detectSingleCommand(name: string): Promise<{ installed: boolean; path: string }> {
  ...
  try {
    ...
    // 排除自有 CLI wrapper（内容含任一标记字符串，新旧 marker 都认）
    try {
      const content = fs.readFileSync(resolvedPath, "utf-8");
      if (CLI_MARKERS.some((m) => content.includes(m))) {
        return { installed: false, path: "" };
      }
    } catch {
      // 二进制文件或无法读取，视为非自有 wrapper
    }
