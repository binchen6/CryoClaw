...
// 读取或生成持久化 device ID（委托给 cryoclaw-config 统一管理）
...
// DataFinder `header.custom` 用的公共属性：只放 header 标准字段之外、CryoClaw 独有的字段
...
// 把 CryoClaw 的 UUID device-id 转成 DataFinder device_id 要求的十进制字符串。
...
// 熵从 64 bit 降到 63 bit，生日碰撞阈值仍在 30 亿设备级，对 CryoClaw 规模完全够用。
...
export function createVolcanoSink(config: VolcanoConfig): SinkConfig {
  ...
  return {
    ...
    endpoints,
    buildPayload: (event, eventProps) => ({
      // CryoClaw 没有账号体系，user_unique_id 留空；把本地 UUID 转成 uint64 填到 header.device_id，
      // DataFinder 按纯设备维度识别。合法 uint64 是服务端接受空 user_unique_id 的唯一条件。
      user: { user_unique_id: "" },
      header: {
        app_id: config.appId,
        // app_name 随品牌更名从 oneclaw 改为 cryoclaw（口径变更：更名前后数据在看板上
        // 会按 app_name 分成两路）；需与 DataFinder 后台注册值保持一致。
        app_name: "cryoclaw",
        app_version: app.getVersion(),
    ...
    headers: {
      ...
      "User-Agent": `CryoClaw/${app.getVersion()}`,
    },
    requestTimeoutMs: config.requestTimeoutMs,
    retryDelaysMs: config.retryDelaysMs,
