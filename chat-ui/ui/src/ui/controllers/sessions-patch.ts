...
//  2) 不引入事件元字段（ts/phase/messageId/messageSeq/sessionKey/agentId/key）；
//  3) 不新增行上原本没有的字段——防事件特有字段污染行结构；
//  4) 无任何行字段可合并时返回 null——删除类 sessions.changed 只带元字段、
//     不携带行快照字段，此时回落全量重拉让被删会话立即消失，
//     而不是等下一次 30s ticker 才收敛。
//
// 已知边界（有意不处理，等下一次全量重拉收敛）：
//  ① 被删除的会话若事件带行字段仍会残留，等下一次全量重拉收敛（无行字段时按 4) 立即收敛）；
//  ② 新会话不进列表；
//  ③ 归档会话的事件可能 patch 到被过滤的行——无害。
//
// 事件元字段：这些是广播点附加的定位/相位信息，不属于行快照，不写进行。
// key 也在其中：行身份不靠事件改，防未来事件携带 key 覆盖行身份。
...
export function applySessionsChangedPatch(
  ...
  //    命中字段浅覆盖。用 Object.hasOwn 只认行自有属性，避免原型链属性（toString 等）误命中。
  ...
  // 无任何行字段被应用：事件只带元字段（如删除类事件），就地合并无意义，
  // 返回 null 让调用方回落全量重拉立即收敛。
  if (applied === 0) {
    return null;
  }
