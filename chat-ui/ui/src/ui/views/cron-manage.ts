...
export type CronManageProps = {
  ...
  /** 可选模型清单（provider/model key），供 agentTurn 任务指定模型 */
  ...
  return html`
    <div class="cm-detail">
      ...
      <div class="cm-detail__meta">
        ...
        <div class="cm-detail__meta-row">
          <span class="cm-detail__meta-label">${t("cron.status")}</span>
      ...
      <div class="cm-detail__history">
        ...
        ${props.runsLoading
          ? html`<div class="muted">${t("cron.runsLoading")}</div>`
          : sorted.length === 0
  ...
  return html`
    <div class="cm-detail">
      ...
      <div class="cron-form">
        ...
        <label class="cron-form__field">
          <span class="cron-form__label">${t("cron.form.model")}</span>
          <select class="cron-form__select" .value=${form.payloadModel}
            @change=${(e: Event) => props.onFormChange({ payloadModel: (e.target as HTMLSelectElement).value })}>
            <option value="">${t("cron.form.modelDefault")}</option>
            ${renderConfiguredModelOptions(props.models, loadModelOrg(), form.payloadModel || undefined)}
          </select>
        </label>
        <div class="cron-form__row">
          <label class="cron-form__field cron-form__field--grow">
    ...
    <div class="cm-layout">
      ...
      <div class="cm-list">
        <div class="cm-list__top">
          <h2 class="cm-list__title panel__title">${t("cron.title")}</h2>
        </div>
