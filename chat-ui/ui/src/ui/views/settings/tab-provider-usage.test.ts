...
const labels = (locale: "zh" | "en") => ({
  ...
  resetHours: locale === "zh" ? "{n}小时后重置" : "{n}h reset",
  resetMinutes: locale === "zh" ? "{n}分钟后重置" : "{n}m reset",
  resetSoon: locale === "zh" ? "即将重置" : "resetting soon",
});

test("computeRateWindowLabel converts 300 MINUTE to '5 小时用量' (zh)", () => {
  const limits = [{ window: { duration: 300, timeUnit: "MINUTE" } }];
  assert.equal(computeRateWindowLabel(limits, labels("zh")), "5 小时用量");
});

test("computeRateWindowLabel converts 300 MINUTE to '5h usage' (en)", () => {
  const limits = [{ window: { duration: 300, timeUnit: "MINUTE" } }];
  assert.equal(computeRateWindowLabel(limits, labels("en")), "5h usage");
});

test("computeRateWindowLabel: MINUTE not divisible by 60 stays in minutes", () => {
  const limits = [{ window: { duration: 30, timeUnit: "MINUTE" } }];
  assert.equal(computeRateWindowLabel(limits, labels("zh")), "30 分钟用量");
  assert.equal(computeRateWindowLabel(limits, labels("en")), "30m usage");
});

test("computeRateWindowLabel: HOUR is used directly", () => {
  const limits = [{ window: { duration: 1, timeUnit: "HOUR" } }];
  assert.equal(computeRateWindowLabel(limits, labels("zh")), "1 小时用量");
  assert.equal(computeRateWindowLabel(limits, labels("en")), "1h usage");
});
...
test("computeRateWindowLabel: reads from limits[0] directly when no window", () => {
  ...
  assert.equal(computeRateWindowLabel(limits, labels("zh")), "2 小时用量");
});
...
test("computeRateWindowLabel falls back when limits empty", () => {
  assert.equal(computeRateWindowLabel([], labels("zh")), RATE_FALLBACK_ZH);
  assert.equal(computeRateWindowLabel([], labels("en")), RATE_FALLBACK_EN);
});

test("computeRateWindowLabel falls back when window metadata missing", () => {
  assert.equal(computeRateWindowLabel([{ used: "10", limit: "100" }], labels("zh")), RATE_FALLBACK_ZH);
});

test("computeRateWindowLabel falls back when duration is 0 or negative", () => {
  assert.equal(
    computeRateWindowLabel([{ window: { duration: 0, timeUnit: "HOUR" } }], labels("zh")),
    RATE_FALLBACK_ZH,
  );
  assert.equal(
    computeRateWindowLabel([{ window: { duration: -5, timeUnit: "HOUR" } }], labels("zh")),
    RATE_FALLBACK_ZH,
  );
});
...
test("computeRateWindowLabel falls back when timeUnit unknown", () => {
  assert.equal(
    computeRateWindowLabel([{ window: { duration: 5, timeUnit: "DAY" } }], labels("zh")),
    RATE_FALLBACK_ZH,
  );
});
...
test("formatResetText: '5小时后重置' / '5h reset' — no '重置于' prefix", () => {
  assert.equal(formatResetText(3600 * 5, labels("zh")), "5小时后重置");
  assert.equal(formatResetText(3600 * 5, labels("en")), "5h reset");
});

test("formatResetText: minutes when under an hour", () => {
  assert.equal(formatResetText(60 * 30, labels("zh")), "30分钟后重置");
  assert.equal(formatResetText(60 * 30, labels("en")), "30m reset");
});

test("formatResetText: 0 returns empty string (no reset row)", () => {
  assert.equal(formatResetText(0, labels("zh")), "");
  assert.equal(formatResetText(-5, labels("en")), "");
});
...
test("deriveUsageView: percentage card displays whole percent", () => {
  ...
  const view = deriveUsageView(data, labels("zh"));
...
test("deriveUsageView: reset text has no 重置于 prefix", () => {
  ...
  // English variant
  const enView = deriveUsageView(data, labels("en"));
...
test("deriveUsageView: tolerates non-object payload", () => {
  const empty = deriveUsageView(null, labels("zh"));
