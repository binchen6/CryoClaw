...
| 测试 | — | 431 个用例全量回归（vitest + node:test + typecheck），0 fail 为硬指标 |
...
- **测试**：vitest（主进程单测）+ node:test（编译产物/脚本）+ chat-ui typecheck 与单测 + scripts 用例，`npm test` 一键全量。
