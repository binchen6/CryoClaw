...
### 通道 A — CryoClaw 手册 + FAQ（能力 / 边界 / 设置 / 故障排查 / 产品 meta）
...
#### (2) CryoClaw 产品 meta 问题

针对 CryoClaw 客户端自身行为的"为什么……"——**禁止用通用 LLM 常识顶替**。
...
| 问"怎么换模型 / 开机启动怎么关" → 脑补 UI 路径 | 所有 CryoClaw 设置必须查手册 |
| 问"为什么卡了" → 答"可能是网络 / 模型问题" | 故障排查走通道 A（manual index → FAQ） |

## 何时不用查

- 与 CryoClaw/OpenClaw 产品无关的 LLM 原生任务（闲聊、写作、翻译、总结、代码生成、通用技术问答）
