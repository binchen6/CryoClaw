...
function readRemoteLatestVersion(packageName, options = {}) {
  ...
  // 防止后续 execFileSync 参数被 cmd.exe 二次解析时注入 shell 元字符。
  if (typeof packageName !== "string" || !/^[a-zA-Z0-9@/._-]+$/.test(packageName)) {
    if (logError) logError(`包名非法，跳过远端版本检查: ${String(packageName).slice(0, 100)}`);
    return "";
  }

  try {
    // Windows 上 npm 是 npm.cmd（批处理脚本），Node 直接 spawn .cmd 需经 cmd.exe 解释；
    // 显式调用 cmd.exe /c npm 避免 shell:true+args 的 DEP0190 警告与拼接注入风险。
    let cmd;
    let args;
    if (process.platform === "win32") {
      cmd = "cmd.exe";
      args = ["/c", "npm", "view", packageName, "version", "--json"];
    } else {
      cmd = "npm";
