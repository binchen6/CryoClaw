...
function loadAnalyticsModule(options = {}) {
  const {
    config = null,
    resourcesPath = "/tmp/cryoclaw-resources",
    appPath = "/Applications/CryoClaw.app/Contents/Resources/app.asar",
    fetchImpl = async () => {
...
test("createVolcanoSink.buildPayload 输出 DataFinder 期望的信封结构", () => {
  ...
  // init() 负责把 deviceId 从 cryoclaw-config mock 拉进模块作用域
  mod.init();
  ...
  try {
    ...
    assert.match(sink.headers["User-Agent"], /^CryoClaw\//);
    ...
    assert.equal(payload.header.app_name, "cryoclaw");
