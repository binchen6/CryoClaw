...
function resolveTargetId(context) {
  ...
  const fromContext = `${platform}-${arch}`;
  const fromEnv = process.env.CRYOCLAW_TARGET;
  // env 与 electron-builder 上下文不一致说明打包命令串错了目标，直接失败避免注错资源
  if (fromEnv) {
    if (fromEnv !== fromContext) {
      throw new Error(
        `[afterPack] CRYOCLAW_TARGET=${fromEnv} 与打包上下文 ${fromContext} 不一致，请检查打包命令`
      );
    }
    return fromEnv;
  }
...
exports.default = async function afterPack(context) {
  ...
  // extensions-mirror/ 是 CryoClaw 第三方 channel plugin 的镜像源目录。
  ...
  // ── 内核升级器（kernel-update.mjs + kernel-dist-patch.js + node_modules） ──
  const updaterSrc = path.join(sourceBase, "updater");
  if (fs.existsSync(updaterSrc)) {
    const updaterDest = path.join(targetBase, "updater");
    copyDirSync(updaterSrc, updaterDest);
    console.log(`[afterPack] 已注入 updater/ → ${path.relative(appOutDir, updaterDest)}`);
  } else {
    console.log(`[afterPack] ⚠ 未找到 updater/ 资源，内核升级功能不可用`);
  }
