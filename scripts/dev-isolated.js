...
const isolatedConfig = path.join(stateDir, "cryoclaw.config.json");
if (!withSetup && !fs.existsSync(isolatedConfig)) {
  const home = process.platform === "win32" ? process.env.USERPROFILE : process.env.HOME;
  const mainStateDir = path.join(home || "", ".openclaw");
  // 新配置优先；上一代 oneclaw.config.json 也认（read fallback 同口径）
  const mainConfig = ["cryoclaw.config.json", "oneclaw.config.json"]
    .map((name) => path.join(mainStateDir, name))
    .find((p) => fs.existsSync(p));
  const mainOpenclaw = path.join(mainStateDir, "openclaw.json");

  // cryoclaw.config.json — 仅在源配置存在时复制，否则自然进入 Setup Wizard
  if (mainConfig) {
    fs.copyFileSync(mainConfig, isolatedConfig);
    console.log(`[dev-isolated] 已复制 ${mainConfig}`);
  }
...
// Windows 下 npm.cmd 必须经 shell 执行（CVE-2024-27980 后 spawn .cmd 不带 shell 会 EINVAL）
const child = spawn(npmCmd, ["run", "dev"], { cwd, stdio: "inherit", env, shell: isWin });
child.on("error", (err) => {
  console.error(`[dev-isolated] 启动 npm run dev 失败: ${err.message}`);
  process.exit(1);
});
